...
const getSkillSubcategories = async (categoryId: string): Promise<AutocompleteOption[]> => {
  await wait();
...
const getSkills = async (subcategoryId: string): Promise<AutocompleteOption[]> => {
  await wait();
  for (const category of mockSkillCategories) {
    const subcategory = category.children.find((sub) => sub.value === subcategoryId);
    if (subcategory) {
