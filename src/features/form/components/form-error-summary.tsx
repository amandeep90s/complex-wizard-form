...
import { ErrorMessage, formatErrors } from '@/utils/formatErrors';
...
const FormErrorSummary = () => {
  ...
  const handleErrorClick = (field: string) => {
    ...
    try {
      ...
      console.log(`No element found for field: ${formFieldName}`);
    } catch (error) {
  ...
  const groupedErrors = formattedErrors.reduce((acc: Record<string, ErrorMessage[]>, error) => {
    if (error.category) {
    ...
    <Alert
      ...
      <List dense>
        {groupedErrors['general']?.map(({ label, message, field }, index) => (
          <ListItem
            key={`general-${index}-${field}`}
            sx={{ px: 0, cursor: 'pointer', '&:hover': { textDecoration: 'underline' } }}
        ...
        {Object.entries(groupedErrors).map(([category, errors]) => {
          ...
          return (
            <div key={category}>
              <ListItem sx={{ px: 0, fontWeight: 'bold' }}>{category}:</ListItem>
              {errors.map(({ label, field, message, index }, idx) => (
                <ListItem
                  key={`${category}-${index}-${idx}`}
                  sx={{ px: 2, cursor: 'pointer', '&:hover': { textDecoration: 'underline' } }}
                  onClick={() => handleErrorClick(field)}>
