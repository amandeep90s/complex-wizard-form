...
type FormatType = 'number' | 'phoneNumber' | 'currency' | 'socialSecurity' | undefined;
...
type TextFieldProps<T extends FieldValues> = Omit<MuiTextFieldProps, 'name' | 'error' | 'helperText'> & {
  name: Path<T>;
  format?: FormatType;
};
...
const createNumberFormat = (
  ...
  return forwardRef<HTMLInputElement, CustomNumberFormatProps>(function NumberFormat(props, ref) {
...
const TextField = forwardRef(
  <T extends FieldValues>({ name, format, sx, ...textFieldProps }: TextFieldProps<T>, ref: Ref<HTMLInputElement>) => {
...
) as <T extends FieldValues>(props: TextFieldProps<T> & { ref?: Ref<HTMLInputElement> }) => ReactElement;
