...
type MenuProps<T extends FieldValues> = Omit<MuiMenuItemProps, 'name' | 'error' | 'value'> & {
  name: Path<T>;
  options: Option[];
...
const Menu = forwardRef(
  <T extends FieldValues>(
    { name, options, MenuItemProps, className, renderLabel, sx, onChange, ...props }: MenuProps<T>,
                ...
                <MenuItem
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    ...
                    {renderLabel ? (
                      ...
                      <Typography sx={{ paddingX: 1, textAlign: 'left' }}>{option.label}</Typography>
                    )}
                  </Box>
                </MenuItem>
              ))}
...
) as <T extends FieldValues>(props: MenuProps<T> & { ref?: Ref<HTMLLIElement> }) => ReactElement;
