...
type AutocompleteProps<T extends FieldValues, Multiple extends boolean = false> = Omit<
  MuiAutocompleteProps<AutocompleteOption, Multiple, false, false>,
  'renderInput' | 'onChange' | 'options' | 'multiple'
> & {
  name: Path<T>;
  textFieldProps?: Omit<React.ComponentProps<typeof MuiTextField>, 'name' | 'error' | 'helperText'>;
    ...
    return (
      <Controller
        ...
        render={({ field: { onChange, value, ...field }, fieldState: { error } }) => {
          const getValue = (): AutocompleteValue<AutocompleteOption, Multiple, false, false> => {
            if (multiple) {
              return (options ?? []).filter((option) =>
                Array.isArray(value) ? value.includes(option.value) : false
              ) as AutocompleteValue<AutocompleteOption, Multiple, false, false>;
            }
            return ((options ?? []).find((option) => option.value === value) || null) as AutocompleteValue<
              AutocompleteOption,
              Multiple,
              false,
              false
          ...
          return (
            <MuiAutocomplete<AutocompleteOption, Multiple, false, false>
              ...
              onChange={(_, newValue) => {
                if (multiple) {
                  const values = (newValue as AutocompleteOption[]).map((option) => option.value);
                  onChange(values);
                  if (onOptionSelect) {
                    (onOptionSelect as (options: AutocompleteOption[]) => void)(newValue as AutocompleteOption[]);
                  }
                } else {
                  const singleValue = newValue as AutocompleteOption | null;
                  onChange(singleValue?.value ?? '');
                  if (onOptionSelect) {
                    (onOptionSelect as (option: AutocompleteOption | null) => void)(singleValue);
                  }
                }
              }}
              renderInput={(params) => (
