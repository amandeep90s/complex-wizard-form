...
export const formatErrors = <T extends Record<string, unknown>>(errors: FieldErrors<T>): ErrorMessage[] => {
  ...
  const processErrors = (
    ...
    Object.entries(obj).forEach(([key, value]) => {
      ...
      const arrayMatch = RegExp(/\[(\d+)\]/).exec(currentField);
      const arrayIndex = arrayMatch ? parseInt(arrayMatch[1]) : undefined;

      const categoryName = currentField.split('[')[0];
      const categoryLabel = d[categoryName as keyof typeof d] || humanizeFieldName(categoryName);
